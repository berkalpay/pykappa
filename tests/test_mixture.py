...
def test_instantiate_pattern_one_component(test_str):
    ...
    assert pattern.components[0].isomorphic(next(iter(mixture.components)))
...
def test_find_embeddings_one_component(test_case):
    ...
    mixture = Mixture()
    for _ in range(n):
        mixture._add(mixture_pattern, n_copies)
...
def test_embeddings_in_component():
    ...
    mixture._add("A(x[.])", n_copies=2)
    mixture._add("A(x[1]), B(x[1])")
...
def test_component_constraint_weights_are_updated_incrementally():
    ...
    for _ in range(30):
        ...
        for rule in system.rules.values():
            ...
            assert rule._component_weight() == rule.n_embeddings(system.mixture)
