...
class Component(Counted):
    ...
    def _embedding_cache_info(
        self,
    ) -> tuple[int, frozenset[tuple[str, str | None]]]:
        """Return the re-embedding radius and state-sensitive agent sites."""
        dependencies: set[tuple[str, str | None]] = {
            (agent.type, None) for agent in self
        }
        dependencies.update(
            (agent.type, site.label)
            for agent in self
            for site in agent
            if site._stated or site.partner not in ("?", "#")
        )
        return self.diameter, frozenset(dependencies)
